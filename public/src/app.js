...
- Seems like too much business logic for a FE library, refactor to address
 */
...
var App = React.createClass({
	...
	render: function() {
		...
		var form = <Form changePage={this.showPage} updateScore={this.updateScore} />;
		var results = <Results changePage={this.showPage} score={this.state.score} />;
		return (
		  <div>
...
var Form = React.createClass({
	...
	validateForm : function() {
		...
		if ( Object.keys(this.state.validAnswers).length === this.state.questionsCount ) {
		...
		return (
			<div>
				...
				<div className="well">
					...
					{ results.recommend ? <Recommendation /> : <p>No further action is required</p> }
				</div>
			</div>
		)
	}
...
var TherapistList = React.createClass({
	...
	render : function() {
		...
		var therapists = test.professionals.map(function(pro, index) {
			return <a href="#" key={index} item={index} onClick={this.updateActiveItem} className={this.state.activeItem === index ? active : inactive}>{pro}</a>
		}, this);
...
var RequestSuccess = React.createClass({
	render : function() {
		return (
			<div className="alert alert-success">
			  <strong>Well done!</strong> You successfully submitted a request.  You will be contacted shortly.
			</div>
		)
	}
});
